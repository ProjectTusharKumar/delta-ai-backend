...
def chat():
    ...
    for i, q in enumerate(queries, 1):
        filters = extract_query_filters(q)
        if filters:
            # Choose projection based on filter type
            ...
            # Field-specific semantic search logic
            # Extract parenthesized phrases as NON_PERSON_WORDS for this query
            ...
            # Remove ignored words for search text
            words = [w for w in q.split() if w.lower() not in IGNORED_WORDS]
            # Remove schema words and dynamic NON_PERSON_WORDS from words list
